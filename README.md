...
* **Основной сценарий:**
  1. Клиент нажимает кнопку «Создать новую заявку».
  2. Заполняет поля «Тема» и «Описание проблемы».
  3. Нажимает кнопку «Отправить».
  4. Система отправляет текст на классификацию ИИ-агенту.
  5. Заявке присваивается расчетный приоритет, клиент видит подтверждение создания заявки со статусом «Новая».
...
* **Основной сценарий:**
  1. Модуль осуществляет предобработку текста (токенизация, лемматизация, удаление стоп-слов).
  2. Текст векторизуется и подается на вход классификационной модели.
  3. Модель вычисляет распределение вероятностей по четырем классам приоритета.
  4. Модуль возвращает класс с наибольшей вероятностью и значение уверенности модели.
  5. Если уровень уверенности ниже заданного порога, система помечает приоритет как «Требует подтверждения».
...
* **Основной сценарий:**
  1. Оператор видит список входящих заявок, отсортированных по приоритету (от Наивысшего к самого низкого).
  2. Выбирает верхнюю заявку в очереди и берет её в работу (статус меняется на «В работе»).
  3. При необходимости несоответствия реальной проблемы расчетному значению оператор может вручную переопределить уровень приоритета (изменение логируется системой).
