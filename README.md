...
* **Материалы испытаний и результаты:**
  * Программа и методика испытаний (сценарии тестов, отчет о достижении метрики F1-score ≥ 0.75).
  * Исходный программный код всех компонентов системы в Git-репозитории с обученными весами ML-модели.

### 9.2 Требования к оформлению
* Язык оформления документации и комментариев в коде — русский (допускаются общепринятые технические термины и англоязычные наименования API-методов).
* Текстовые отчетные материалы формируются в электронном виде (форматы PDF, Markdown, DOCX) с соблюдением требований к оформлению учебных и курсовых проектов ВолгГТУ и ГОСТ 19.xxx / 34.xxx.

---

## 10. Требования к Исполнителю

Разработка выполняется проектной группой студентов группы ИВТ-262. Члены команды должны обладать следующими компетенциями:
* **Архитектура и Backend-разработка:** владение языком Python (FastAPI/Django), проектирование реляционных баз данных (PostgreSQL), разработка RESTful API, реализация аутентификации на основе JWT.
* **Машинное обучение и NLP:** знание методов предобработки текстов (токенизация, лемматизация, удаление шума), опыт обучения классификаторов с использованием библиотек scikit-learn / PyTorch / Transformers, расчет метрик Precision, Recall и F1-score.
* **Frontend-разработка:** разработка адаптивных веб-интерфейсов (HTML5, CSS3, JavaScript/TypeScript, React/Vue), работа с асинхронными запросами к API.
* **DevOps и системное окружение:** опыт работы с системой контроля версий Git, контейнеризацией на базе Docker и написанием сценариев развертывания `docker-compose.yml`.
